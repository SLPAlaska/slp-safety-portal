...
route = r"""export const maxDuration = 300
...
async function processSlides(supabaseAdmin, job, course_id, mode) {
  ...
  for (const slide of slides || []) {
    try {
      ...
      if (!aiRes.ok) {
        console.error('Anthropic error slide ' + sr + ':', aiRes.status, await aiRes.text())
        processed++
...
export async function POST(request) {
  ...
  try {
    ...
    // Return job_id immediately — process slides in background after response is sent
    after(async () => {
      try {
        await processSlides(supabaseAdmin, job, course_id, mode)
      } catch (err) {
...
path = r"C:\Users\brian\OneDrive\SLP HARD DRIVE\00000-2026\GitHub Safety Portal\app\api\lms\ai-generate\route.js"
with open(path, 'w', encoding='ascii', errors='replace') as f:
    f.write(route)

print("route.js written successfully")

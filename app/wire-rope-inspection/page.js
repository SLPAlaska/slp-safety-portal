...
export default function WireRopeInspection() {
  ...
  const checkDuplicateInspection = async (ropeId) => {
    ...
    let data = []; try { data = (await fieldData('recent_check', { table: 'wire_rope_inspections', id: ropeId })).rows; } catch (e) { console.error(e); } const error = null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await safeSubmit({
        table: 'wire_rope_inspections',
        data: { ...formData },
        photoRef: photoRef,
        formType: 'wire-rope-inspection'
      });
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)', overflow: 'hidden' }}>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '25px' }}>
            ...
            {/* Photo Documentation */}
            <MultiPhotoUpload ref={photoRef} formType="wire-rope-inspection" />

...
// ═══════════════════════════════════════════════════════════════════════
// SLP ALASKA — WEATHER & PHASE CONDITIONS v5
// ─ Windy.com embedded map (left) — real-time animated weather
// ─ NOAA hardcoded stations for Alaska accuracy (right panel)
...
const NOAA_STATIONS={"Deadhorse / Prudhoe Bay":"PASC","Kuparuk":"PASC","Alpine (CD-1)":"PALU","Nuiqsut":"PAQT","West Harrison Bay":"PASC","Oliktok Point":"POLI","Utqia\u0121vik (Barrow)":"PABR","Umiat":"PAUM","Pt. Thompson":"PASC","Badami":"PASC","Kenai":"PAEN","Soldotna":"PAEN","Nikiski":"PAEN","Anchorage":"PANC","Fairbanks":"PAFA","Valdez":"PAVD","Homer":"PAHO","Kodiak":"PADQ","Bethel":"PABE","Nome":"PAOM","Dillingham":"PADL"};
...
// ═══════════════════════════════════════════════════════════════════════
// WINDY EMBED — builds iframe URL for selected location
// ═══════════════════════════════════════════════════════════════════════
function buildWindyUrl(lat, lon, zoom) {
  return `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=%C2%B0F&metricWind=mph&zoom=${zoom||7}&overlay=wind&product=ecmwf&level=surface&lat=${lat}&lon=${lon}&marker=true&message=true`;
}
...
async function fetchOpenMeteo(lat,lon){
  try{
    ...
    const res=await fetch(url);if(!res.ok)throw new Error("Open-Meteo API error");
    ...
    const d=data.daily;
    const forecast=d.time.map((t,i)=>({date:t,weather_code:d.weather_code[i],description:WMO[d.weather_code[i]]||"",icon:WMO_ICON[d.weather_code[i]]||"",temp_max:d.temperature_2m_max[i],temp_min:d.temperature_2m_min[i],precip:d.precipitation_sum[i],wind_max:d.wind_speed_10m_max[i],wind_dir:d.wind_direction_10m_dominant[i]}));
...
async function fetchNOAAStation(stationId){
  try{
    const res=await fetch(`https://api.weather.gov/stations/${stationId}/observations/latest`,{headers:{"User-Agent":"SLPAlaskaSafetyPortal/5.0"}});
    if(!res.ok)return null;const data=await res.json();const p=data.properties;
    const tc=p.temperature?.value;const ws=p.windSpeed?.value;const vm=p.visibility?.value;
    const obsTime=p.timestamp?new Date(p.timestamp):null;const ageHrs=obsTime?(new Date()-obsTime)/3600000:999;
...
async function fetchWeather(lat,lon,locationName){
  ...
  const[openMeteo,noaa]=await Promise.all([fetchOpenMeteo(lat,lon),stId?fetchNOAAStation(stId):Promise.resolve(null)]);
  if(noaa&&openMeteo.current){
    const merged={...openMeteo.current,temp_f:noaa.temp_f,wind_mph:noaa.wind_mph??openMeteo.current.wind_mph,wind_dir:noaa.wind_dir??openMeteo.current.wind_dir,humidity:noaa.humidity??openMeteo.current.humidity,visibility_miles:noaa.visibility_miles??openMeteo.current.visibility_miles,description:noaa.description!=="N/A"?noaa.description:openMeteo.current.description,station:noaa.station,dataAge:noaa.dataAge,source:`NOAA ${stId}`,timestamp:noaa.timestamp};
    merged.wind_chill=calcWindChill(merged.temp_f,merged.wind_mph);merged.frostbite_time=frostbiteTime(merged.wind_chill);
...
async function searchLocations(query){
  try{const res=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=8&language=en&format=json`);
    if(!res.ok)return[];const data=await res.json();
    return(data.results||[]).map(r=>({name:[r.name,r.admin1,r.country].filter(Boolean).join(", "),lat:r.latitude,lon:r.longitude,country:r.country_code}));
  }catch{return[];}
}
...
// UI COMPONENTS
// ═══════════════════════════════════════════════════════════════════════
function MetricBox({label,value,sub,alert:isA,icon}){return(
  <div style={{padding:"10px 12px",borderRadius:"8px",background:isA?"rgba(239,68,68,0.1)":"rgba(255,255,255,0.04)",border:`1px solid ${isA?"rgba(239,68,68,0.3)":"rgba(255,255,255,0.06)"}`}}>
    <div style={{fontSize:"10px",color:"#8892a4",textTransform:"uppercase",letterSpacing:"0.05em",display:"flex",alignItems:"center",gap:"4px"}}>{icon&&<span style={{fontSize:"12px"}}>{icon}</span>}{label}</div>
    <div style={{fontSize:"18px",fontWeight:800,color:isA?"#ef4444":"#fff",marginTop:"2px"}}>{value}</div>
    {sub&&<div style={{fontSize:"10px",color:isA?"#f97316":"#8892a4",marginTop:"2px"}}>{sub}</div>}
  </div>);}

function ThresholdRow({label,value,thresholds,alert:isA}){return(
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",borderRadius:"6px",background:isA?"rgba(249,115,22,0.08)":"rgba(255,255,255,0.03)",border:`1px solid ${isA?"rgba(249,115,22,0.2)":"rgba(255,255,255,0.04)"}`,gap:"8px"}}>
    <div><div style={{fontSize:"11px",fontWeight:600,color:isA?"#f97316":"#e2e8f0"}}>{label}: {value}</div>
    <div style={{fontSize:"9px",color:"#64748b"}}>{thresholds}</div></div>
    {isA&&<span style={{fontSize:"14px"}}>{"\u26A0\uFE0F"}</span>}
  </div>);}
...
export default function WeatherPage(){
  ...
  const[windyUrl,setWindyUrl]=useState(buildWindyUrl(62.0,-152.0,4)); // Default: Alaska overview
  ...
  const selectLocation=useCallback(async(l)=>{
    setLoc(l);setLoading(true);setError(null);setWeather(null);setSearchResults([]);setSearchText("");setShowAK(true);
    // Update Windy map to center on selected location
    const isNS=AK_LOCS.find(a=>a.name===l.name)?.ns;
    const zoom=isNS?8:(AK_LOCS.find(a=>a.name===l.name)?7:6);
    setWindyUrl(buildWindyUrl(l.lat,l.lon,zoom));
    const data=await fetchWeather(l.lat,l.lon,l.name);
    if(data.error)setError(data.error);else{setWeather(data);setTab(AK_LOCS.find(a=>a.name===l.name)?"ash":"current");}
    setLoading(false);
  },[]);
  ...
  const handleSearch=useCallback((val)=>{
    ...
    searchTimeout.current=setTimeout(async()=>{
      ...
      setSearchResults([...akMatch.map(l=>({name:l.name,lat:l.lat,lon:l.lon,isAK:true,ns:l.ns})),
        ...global.filter(g=>!akMatch.find(a=>Math.abs(a.lat-g.lat)<0.1&&Math.abs(a.lon-g.lon)<0.1)).map(g=>({...g,isAK:false}))]);
    },350);
  },[]);
  ...
  return(
    <div style={{background:"linear-gradient(145deg,#080f1e 0%,#0c1a2e 50%,#080f1e 100%)",borderRadius:"16px",border:"1px solid rgba(255,255,255,0.06)",overflow:"hidden",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#e2e8f0",width:"100%",maxWidth:"1100px",margin:"0 auto"}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:800px){.wp-main{flex-direction:column!important}.wp-map{flex:none!important;height:350px!important;width:100%!important}.wp-panel{border-left:none!important;border-top:1px solid rgba(255,255,255,0.06)!important}}.wg-chip{padding:3px 10px;border-radius:14px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.03);color:#8892a4;font-size:11px;cursor:pointer;white-space:nowrap;transition:all 0.15s}.wg-chip:hover{background:rgba(74,158,255,0.12);color:#4a9eff}.wg-chip-active{background:rgba(74,158,255,0.15)!important;color:#4a9eff!important;border-color:rgba(74,158,255,0.3)!important}.wg-tab{flex:1;padding:10px;background:none;border:none;border-bottom:2px solid transparent;color:#64748b;font-size:12px;cursor:pointer;transition:all 0.15s}.wg-tab:hover{color:#94a3b8}.wg-tab-active{color:#4a9eff!important;border-bottom-color:#4a9eff!important;font-weight:700!important}.wg-sr{padding:8px 12px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,0.03);transition:background 0.1s;font-size:12px}.wg-sr:hover{background:rgba(74,158,255,0.08)}`}</style>
      ...
      <div style={{padding:"14px 20px 10px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"8px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          <div>
            ...
            <div style={{fontSize:"10px",color:"#64748b"}}>Windy.com Live Map {"\u2022"} NOAA Station Data {"\u2022"} ASH Book 2026</div>
          </div>
        </div>
      ...
      {/* Body */}
      <div className="wp-main" style={{display:"flex",flexDirection:"row",minHeight:"520px"}}>

        {/* LEFT: Windy Map */}
        <div className="wp-map" style={{flex:"0 0 55%",height:"520px",position:"relative",borderRight:"1px solid rgba(255,255,255,0.06)"}}>
          <iframe
            key={windyUrl}
            src={windyUrl}
        ...
        {/* RIGHT: Data Panel */}
        <div className="wp-panel" style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,maxHeight:"520px"}}>
          ...
          <div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
            ...
            {searchResults.length>0&&(
              <div style={{marginTop:"4px",borderRadius:"8px",border:"1px solid rgba(255,255,255,0.08)",background:"rgba(12,26,46,0.98)",maxHeight:"180px",overflowY:"auto",position:"relative",zIndex:10}}>
                {searchResults.map((r,i)=>(
                  <div key={i} className="wg-sr" onClick={()=>selectLocation({lat:r.lat,lon:r.lon,name:r.name})}>
            ...
            {showAK&&(<div style={{display:"flex",flexWrap:"wrap",gap:"3px",marginTop:"6px",maxHeight:"72px",overflowY:"auto"}}>
              {AK_LOCS.map(l=>(<button key={l.name} className={`wg-chip ${loc?.name===l.name?"wg-chip-active":""}`}
                onClick={()=>selectLocation({lat:l.lat,lon:l.lon,name:l.name})}>{l.ns?"\uD83C\uDFD4\uFE0F":"\uD83D\uDCCD"} {l.name}</button>))}
            </div>)}
          </div>
          ...
          <div style={{flex:1,overflow:"auto"}}>
            {!loc&&!loading&&(
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",color:"#475569",fontSize:"13px",textAlign:"center",padding:"24px",gap:"8px"}}>
                <span style={{fontSize:"36px",opacity:0.2}}>{"\uD83C\uDF21\uFE0F"}</span>
                <div style={{lineHeight:1.6}}>Select an Alaska location above<br/>or search for any place worldwide</div>
                <div style={{fontSize:"10px",color:"#334155",marginTop:"4px"}}>Windy map shows real-time animated weather<br/>Right panel shows station data & ASH Book phases</div>
              </div>)}
            {loading&&(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"160px",gap:"10px",color:"#4a9eff"}}>
              <div style={{width:"18px",height:"18px",border:"2px solid rgba(74,158,255,0.3)",borderTop:"2px solid #4a9eff",borderRadius:"50%",animation:"spin 0.8s linear infinite"}} />Fetching data...</div>)}
            {error&&!loading&&(<div style={{padding:"16px"}}><div style={{color:"#f97316",fontSize:"13px",fontWeight:600}}>{"\u26A0\uFE0F"} {error}</div></div>)}
                ...
                {tab==="current"&&weather.current&&(
                  <div style={{padding:"14px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:"14px",marginBottom:"14px"}}>
                      <div>
                        <div style={{fontSize:"42px",fontWeight:800,color:"#fff",lineHeight:1,letterSpacing:"-1px"}}>{weather.current.temp_f!==null?`${Math.round(weather.current.temp_f)}\u00B0`:"--"}<span style={{fontSize:"18px",fontWeight:400,color:"#8892a4"}}>F</span></div>
                        {weather.current.apparent_f!=null&&<div style={{fontSize:"11px",color:"#64748b",marginTop:"2px"}}>Feels like {Math.round(weather.current.apparent_f)}\u00B0F</div>}
                      </div>
                ...
                {tab==="forecast"&&(
                  <div style={{padding:"10px 14px"}}>
                    ...
                    {weather.forecast.map((p,i)=>(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 10px",marginBottom:"3px",borderRadius:"8px",background:i===0?"rgba(74,158,255,0.06)":"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)"}}>
                  ...
                  <div style={{padding:"10px 14px"}}>
                    ...
                    {pd&&(<div style={{padding:"12px",borderRadius:"10px",background:pd.bg,border:`1px solid ${pd.border}`,marginBottom:"10px"}}>
                      ...
                      {[["\uD83D\uDE97 Travel",pd.travel],["\uD83C\uDFD7\uFE0F Outdoor Work",pd.outdoor],["\u2699\uFE0F Equipment",pd.equipment]].map(([l,t])=>(
                        <div key={l} style={{marginBottom:"6px"}}><div style={{fontSize:"10px",fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:"2px"}}>{l}</div>
                        <div style={{fontSize:"11px",color:"#e2e8f0",lineHeight:1.5}}>{t}</div></div>))}
                    </div>)}
                    ...
                    <div style={{marginTop:"10px",padding:"8px 10px",borderRadius:"8px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.04)"}}>
                      ...
                      {Object.entries(ASH_BOOK_PHASES).map(([k,p])=>(<div key={k} style={{display:"flex",alignItems:"center",gap:"8px",padding:"2px 0",fontSize:"10px",color:k===phase?p.color:"#64748b",fontWeight:k===phase?700:400}}>
                        <span style={{width:"8px",height:"8px",borderRadius:"50%",background:p.color,flexShrink:0}} />{p.label}</div>))}
                    </div>
                  </div>)}
              </div>)}
          </div>

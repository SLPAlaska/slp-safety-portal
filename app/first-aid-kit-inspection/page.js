...
export default function FirstAidKitInspection() {
  ...
  const checkDuplicateInspection = async (kitId) => {
    ...
    let data = []; try { data = (await fieldData('recent_check', { table: 'first_aid_kit_inspections', id: kitId })).rows; } catch (e) { console.error(e); } const error = null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await safeSubmit({
        table: 'first_aid_kit_inspections',
        data: { ...formData },
        photoRef: photoRef,
        formType: 'first-aid-kit-inspection'
      });

...
export default function PPEInspection(){
  ...
  const [formData,setFormData]=useState({
    inspectorName:'',date:new Date().toLocaleDateString('en-CA'),company:'',location:'',employeeName:'',employeeId:'',
  ...
  const handleSubmit=async(e)=>{e.preventDefault();setIsSubmitting(true);
    ...
    try{
      const { error } = await safeInsert('ppe_inspections', [{
        ...
        respiratory_protection_inspected:formData.respiratoryProtectionInspected,respirator_type:formData.respiratorType,facepiece_condition:formData.facepieceCondition,straps_harness:formData.strapsHarness,valves_condition:formData.valvesCondition,cartridge_filter_status:formData.cartridgeFilterStatus,cartridge_expiration:formData.cartridgeExpiration || null,fit_test_current:formData.fitTestCurrent,medical_clearance_current:formData.medicalClearanceCurrent,respiratory_protection_result:formData.respiratoryProtectionResult,

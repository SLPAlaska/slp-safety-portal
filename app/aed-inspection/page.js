...
const AED_MAKES = [
  'Philips', 'ZOLL', 'Physio-Control/Stryker', 'Cardiac Science',
  'Defibtech', 'HeartSine', 'Nihon Kohden', 'Other'
];

export default function AEDInspection() {
  const [formData, setFormData] = useState({
    ...
    // Pads
    adult_pads_present: '',
    adult_pads_expiration: '',
    adult_pads_sealed: '',
    pediatric_pads_present: '',
    pediatric_pads_expiration: '',
    // Supplies
    razor_present: '',
    scissors_present: '',
    gloves_present: '',
  ...
  const checkDuplicateInspection = async (aedId) => {
    if (!aedId || duplicateConfirmed) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await safeInsert('aed_inspections', [sanitized]);
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 50%, #b91c1c 100%)', padding: '20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '50px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '40px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '25px' }}>AED Inspection recorded successfully.</p>
            <button
              onClick={resetForm}
              style={{
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)', overflow: 'hidden' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', color: 'white', padding: '30px 20px', textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: '28px', marginBottom: '5px', textShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>❤️ Monthly AED Inspection</h1>
            <p style={{ fontSize: '16px', opacity: '0.9' }}>Automated External Defibrillator Monthly Check</p>
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '8px', padding: '10px 15px', marginTop: '15px', fontSize: '13px' }}>
              ⚠️ OSHA 29 CFR 1910.151 & AHA/ILCOR Guidelines | Monthly Inspection Required | AEDs Save Lives!
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '25px' }}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #1e3a8a' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>AED ID / Serial Number *</label>
                  <input type="text" name="aed_id" value={formData.aed_id} onChange={handleChange} required
                    placeholder="e.g., AED-001 or S/N 12345"
                    onBlur={() => checkDuplicateInspection(formData.aed_id)}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                  ...
                  {duplicateWarning && !duplicateConfirmed && (
                    <div style={{ background: 'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)', border: '2px solid #f59e0b', borderRadius: '8px', padding: '15px', marginTop: '10px' }}>
                      ...
                      <p style={{ color: '#92400e', fontSize: '13px', marginBottom: '10px' }}>
                        This AED was inspected <strong>{duplicateWarning.daysAgo} days ago</strong> on {duplicateWarning.date} by {duplicateWarning.inspector}.
                      </p>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <button type="button" onClick={() => { setFormData(prev => ({ ...prev, aed_id: '' })); setDuplicateWarning(null); }}
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>AED Make *</label>
                  <select name="aed_make" value={formData.aed_make} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select Make...</option>
                    {AED_MAKES.map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>AED Model</label>
                  <input type="text" name="aed_model" value={formData.aed_model} onChange={handleChange}
                    placeholder="e.g., HeartStart FRx, AED Plus"
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>AED Location Description *</label>
                  <input type="text" name="aed_location_description" value={formData.aed_location_description} onChange={handleChange} required
                    placeholder="e.g., Main Entrance, Break Room, Control Room"
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
              </div>
            </div>

            {/* Unit Status & Accessibility */}
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #dc2626' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="Unit Status Indicator Present" name="unit_status_indicator" />
                <InspectionItem label="Ready Light ON (Green/Blinking)" name="ready_light_on" options={[
                  { value: 'Pass', label: 'Pass - Light ON' },
                  { value: 'Fail', label: 'Fail - Light OFF' }
                ]} />
                <InspectionItem label="Service Light OFF (No Alerts)" name="service_light_off" options={[
                  { value: 'Pass', label: 'Pass - No Alerts' },
                  { value: 'Fail', label: 'Fail - Service Required' }
                ]} />
              </div>
            </div>

            {/* Battery Status */}
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #f59e0b' }}>
              <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#f59e0b', marginBottom: '15px' }}>
                🔋 Battery Status
              </h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Battery Install Date</label>
                  <input type="date" name="battery_install_date" value={formData.battery_install_date} onChange={handleChange}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #7c3aed' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Pediatric Pads Expiration Date</label>
                  <input type="date" name="pediatric_pads_expiration" value={formData.pediatric_pads_expiration} onChange={handleChange}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #0891b2' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="CPR Mask/Barrier Present" name="cpr_mask_present" showNA />
                <InspectionItem label="Towel/Wipes Present" name="towel_wipes_present" showNA />
                <InspectionItem label="Rescue Kit Complete" name="rescue_kit_complete" showNA />
                <InspectionItem label="Quick Reference Guide Present" name="quick_reference_guide" />
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #6366f1' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Next Service Due</label>
                  <input type="date" name="next_service_due" value={formData.next_service_due} onChange={handleChange}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #16a34a' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <select name="overall_condition" value={formData.overall_condition} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select...</option>
                    <option value="Good">Good - Ready for Emergency Use</option>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                    <label style={{
                      ...
                      background: formData.inspection_result === 'Pass - Ready for Use' ? '#dcfce7' : 'white',
                      border: `2px solid ${formData.inspection_result === 'Pass - Ready for Use' ? '#16a34a' : '#e5e7eb'}`,
                      borderRadius: '8px', cursor: 'pointer'
                    }}>
                      <input type="radio" name="inspection_result" value="Pass - Ready for Use" checked={formData.inspection_result === 'Pass - Ready for Use'} onChange={handleChange} required />
                      <span>✅ Pass - Ready for Use</span>
                    </label>
                    <label style={{
                      display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 15px',
                      background: formData.inspection_result === 'Fail - Out of Service' ? '#fee2e2' : 'white',
                      border: `2px solid ${formData.inspection_result === 'Fail - Out of Service' ? '#dc2626' : '#e5e7eb'}`,
                      borderRadius: '8px', cursor: 'pointer'
                    }}>
                      <input type="radio" name="inspection_result" value="Fail - Out of Service" checked={formData.inspection_result === 'Fail - Out of Service'} onChange={handleChange} />
                      <span>❌ Fail - Out of Service</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <select name="issues_found" value={formData.issues_found} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select...</option>
                    <option value="No">No - All Items Pass</option>
                    <option value="Yes">Yes - Issues Noted</option>
                  </select>
                </div>
                {formData.issues_found === 'Yes' && (
                  <div>
                    ...
                    <select name="action_taken" value={formData.action_taken} onChange={handleChange}
                      ...
                      <option value="Replacement Parts Ordered">Replacement Parts Ordered</option>
                      <option value="Battery Replaced">Battery Replaced</option>
                      <option value="Pads Replaced">Pads Replaced</option>
                      <option value="Service Call Scheduled">Service Call Scheduled</option>
                      <option value="Unit Sent for Service">Unit Sent for Service</option>
                      <option value="Supplies Restocked">Supplies Restocked</option>
              ...
              {formData.issues_found === 'Yes' && (
                <div style={{ marginBottom: '15px' }}>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Issue Description</label>
                  <textarea name="issue_description" value={formData.issue_description} onChange={handleChange}
            ...
            {/* Photo Documentation */}
            <MultiPhotoUpload ref={photoRef} formType="aed-inspection" />

...
const LADDER_TYPES = [
  'Step Ladder', 'Extension Ladder', 'Platform Ladder', 'Fixed Ladder',
  'Multi-Position Ladder', 'Combination Ladder', 'Rolling Ladder', 'Attic Ladder', 'Other'
];

const MATERIALS = ['Fiberglass', 'Aluminum', 'Wood', 'Steel', 'Other'];

const HEIGHTS = ['4 ft', '6 ft', '8 ft', '10 ft', '12 ft', '16 ft', '20 ft', '24 ft', '28 ft', '32 ft', '40 ft', 'Other'];

const DUTY_RATINGS = [
  'Type IAA - 375 lbs', 'Type IA - 300 lbs', 'Type I - 250 lbs',
  'Type II - 225 lbs', 'Type III - 200 lbs'
];
...
export default function LadderInspection() {
  const [formData, setFormData] = useState({
    ...
    side_rails_cracks: '',
    side_rails_dents: '',
    side_rails_corrosion: '',
    side_rails_warping: '',
    // Steps/Rungs
    steps_secure: '',
    steps_condition: '',
    steps_slip_resistant: '',
    // Hardware
    hardware_secure: '',
    rivets_bolts: '',
    safety_feet_condition: '',
    safety_feet_grip: '',
    // Step Ladder
    spreaders_function: '',
    spreader_locks: '',
  ...
  const checkDuplicateInspection = async (ladderId) => {
    if (!ladderId || duplicateConfirmed) return;

    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    let data = []; try { data = (await fieldData('recent_check', { table: 'ladder_inspections', id: ladderId })).rows; } catch (e) { console.error(e); } const error = null;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'ladder_id' && value.length >= 3) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await safeSubmit({
        table: 'ladder_inspections',
        data: { ...formData },
        photoRef: photoRef,
        formType: 'ladder-inspection'
      });
  ...
  const showStepLadder = ['Step Ladder', 'Platform Ladder', 'Rolling Ladder', 'Multi-Position Ladder', 'Combination Ladder'].includes(formData.ladder_type);
  const showExtension = ['Extension Ladder', 'Multi-Position Ladder', 'Combination Ladder'].includes(formData.ladder_type);
  const showMultiPosition = ['Multi-Position Ladder', 'Combination Ladder'].includes(formData.ladder_type);
  const showPlatform = ['Platform Ladder', 'Rolling Ladder'].includes(formData.ladder_type);
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 50%, #b91c1c 100%)', padding: '20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '50px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '40px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '25px' }}>Ladder Inspection recorded successfully.</p>
            <button
              onClick={resetForm}
              style={{
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)', overflow: 'hidden' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', color: 'white', padding: '30px 20px', textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: '28px', marginBottom: '5px', textShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>🪜 Monthly Ladder Inspection</h1>
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '8px', padding: '10px 15px', marginTop: '15px', fontSize: '13px' }}>
              ⚠️ OSHA 29 CFR 1910.23 & ANSI A14.1-A14.5 Compliant | Monthly Inspection Required
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '25px' }}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #1e3a8a' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Ladder ID / Tag Number *</label>
                  <input type="text" name="ladder_id" value={formData.ladder_id} onChange={handleChange} required
                    placeholder="e.g., LAD-001"
                    onBlur={() => checkDuplicateInspection(formData.ladder_id)}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                  ...
                  {duplicateWarning && !duplicateConfirmed && (
                    <div style={{ background: 'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)', border: '2px solid #f59e0b', borderRadius: '8px', padding: '15px', marginTop: '10px' }}>
                      ...
                      <p style={{ color: '#92400e', fontSize: '13px', marginBottom: '10px' }}>
                        This ladder was inspected <strong>{duplicateWarning.daysAgo} days ago</strong> on {duplicateWarning.date} by {duplicateWarning.inspector}.
                      </p>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <button type="button" onClick={() => { setFormData(prev => ({ ...prev, ladder_id: '' })); setDuplicateWarning(null); }}
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Storage Location</label>
                  <input type="text" name="storage_location" value={formData.storage_location} onChange={handleChange}
                    placeholder="e.g., Shop A, Warehouse Bay 3"
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Ladder Type *</label>
                  <select name="ladder_type" value={formData.ladder_type} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select Type...</option>
                    {LADDER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Material *</label>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Height / Length *</label>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Duty Rating *</label>
                  <select name="duty_rating" value={formData.duty_rating} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select Duty Rating...</option>
                    {DUTY_RATINGS.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <input type="text" name="manufacturer" value={formData.manufacturer} onChange={handleChange}
                    placeholder="e.g., Werner, Louisville, Little Giant"
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #dc2626' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="No Warping / Twisting" name="side_rails_warping" />
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #0891b2' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="Safety Feet Grip Adequate" name="safety_feet_grip" />
              </div>
            </div>

            {/* Step Ladder Components (Conditional) */}
            {showStepLadder && (
            ...
            {/* Extension Ladder Components (Conditional) */}
            {showExtension && (
              <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #7c3aed' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#7c3aed', marginBottom: '15px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                  ...
                  <InspectionItem label="Locking Mechanisms Secure" name="locking_mechanisms" showNA />
                </div>
              </div>
            )}

            {/* Multi-Position Components (Conditional) */}
            {showMultiPosition && (
              <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #16a34a' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#16a34a', marginBottom: '15px' }}>
            ...
            {/* Platform Ladder Components (Conditional) */}
            {showPlatform && (
              <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #0d9488' }}>
            ...
            {/* Photo Documentation */}
            <MultiPhotoUpload ref={photoRef} formType="ladder-inspection" />

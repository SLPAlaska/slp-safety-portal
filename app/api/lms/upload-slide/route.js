...
export async function POST(request) {
  ...
  try {
    ...
    const { error: uploadError } = await supabaseAdmin.storage
      .from('lms-slides').upload(storagePath, buffer, { contentType: 'image/png', upsert: true })
    if (uploadError) return NextResponse.json({ error: uploadError.message }, { status: 400 })
    const { data: slide, error: dbError } = await supabaseAdmin
      .from('lms_slides')
      .upsert({ course_id, slide_order, image_path: storagePath, speaker_notes: speaker_notes.trim() }, { onConflict: 'course_id,slide_order' })
      .select().single()

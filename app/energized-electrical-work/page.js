...
export default function EnergizedElectricalWork(){
  ...
  const [formData,setFormData]=useState({
    permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),phoneRadio:'',company:'',location:'',equipmentId:'',nominalVoltage:'',workDescription:'',justification:'',
  ...
  const loadOpenPermits=async()=>{try{const{rows}=await fieldData('open_permits',{table:'eew_permits'});setOpenPermits(rows||[]);}catch(e){console.error(e);}};
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      ...
      const{error}=await safeInsert('eew_permits', [{
      ...
      await registerRecordKey('eew_permits',permitNumber,code);
  ...
  const submitCloseout=async()=>{if(!selectedPermit){alert('Select a permit');return;}if(!closeoutCode.trim()){alert('Enter the Close-Out Code from the permit receipt');return;}if(!closeoutData.closeOutWorker||!closeoutData.closeOutSupervisor){alert('Fill required fields');return;}
    setIsSubmitting(true);try{
      const{error}=await safeCloseout('eew_permits',selectedPermit,closeoutCode,{permit_status:'Closed',work_completed:closeoutData.workCompleted?'Yes':'No',area_secured:closeoutData.areaSecured?'Yes':'No',equipment_restored:closeoutData.equipmentRestored?'Yes':'No',barricades_removed:closeoutData.barricadesRemoved?'Yes':'No',time_permit_closed:closeoutData.timePermitClosed||null,close_out_worker:closeoutData.closeOutWorker,close_out_supervisor:closeoutData.closeOutSupervisor,closed_at:new Date().toISOString()});
      if(error)throw error;setCloseoutSuccess(true);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const resetForm=()=>{setFormData({permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),phoneRadio:'',company:'',location:'',equipmentId:'',nominalVoltage:'',workDescription:'',justification:'',shockHazardPresent:'',limitedApproach:'',restrictedApproach:'',prohibitedApproach:'',arcFlashLabel:'',arcFlashBoundary:'',incidentEnergy:'',ppeCategory:'',workingDistance:'',faultCurrent:'',clearingTime:'',hazardRiskCategory:'',arcFlashAbove40:'',arcRatedClothing:'',arcRatingMinimum:'',arcRatedFaceShield:'',arcFlashSuitHood:'',safetyGlasses:'',arcRatedHardHat:'',voltageRatedGloves:'',leatherProtectors:'',voltageRatedTools:'',insulatedMatting:'',hearingProtection:'',voltageRatedSleeves:'',qualifiedPerson:'',jobBriefing:'',testEquipmentVerified:'',barricadesSigns:'',emergencyProcedures:'',safetyWatchAlerter:'',singleLineDiagram:'',rescueEquipment:'',secondQualifiedPerson:'',correctCircuit:'',lotoAttempted:'',properTestEquipment:'',approachBoundariesMarked:'',nonConductiveLadders:'',conductiveItemsRemoved:'',illuminationAdequate:'',workingSpaceClear:'',additionalHazards:{},hazardTask1:'',hazard1:'',mitigation1:'',hazardTask2:'',hazard2:'',mitigation2:'',qualifiedPersonSignature:'',responsibleSupervisor:'',areaOperator:'',startTime:'',expirationTime:'',crewAcknowledgment:''});setSubmitted(false);setSubmittedPermit(null);setPermitCode(null);};
  const resetCloseout=()=>{setCloseoutData({workCompleted:false,areaSecured:false,equipmentRestored:false,barricadesRemoved:false,timePermitClosed:'',closeOutWorker:'',closeOutSupervisor:''});setSelectedPermit(null);setCloseoutSuccess(false);setCloseoutCode('');loadOpenPermits();};

...
export default function HotWorkPermit(){
  ...
  const [formData,setFormData]=useState({
    permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),hotWorkType:'',company:'',location:'',phoneRadio:'',toolsEquipment:'',workLocationDetail:'',workDescription:'',
  ...
  const loadOpenPermits=async()=>{try{const{rows}=await fieldData('open_permits',{table:'hot_work_permits'});setOpenPermits(rows||[]);}catch(e){console.error(e);}};
  ...
  const handleSubmit=async(e)=>{e.preventDefault();setIsSubmitting(true);
    try{
      ...
      const{error}=await safeInsert('hot_work_permits', [{
      ...
      await registerRecordKey('hot_work_permits',permitNumber,code);
  ...
  const submitCloseout=async()=>{if(!selectedPermit){alert('Select a permit');return;}if(!closeoutCode.trim()){alert('Enter the Close-Out Code from the permit receipt');return;}if(!closeoutData.closeOutWorker||!closeoutData.closeOutOperator){alert('Fill required fields');return;}
    setIsSubmitting(true);try{
      const{error}=await safeCloseout('hot_work_permits',selectedPermit,closeoutCode,{permit_status:'Closed',job_completed:closeoutData.jobCompleted?'Yes':'No',work_area_secured:closeoutData.workAreaSecured?'Yes':'No',systems_returned:closeoutData.systemsReturned?'Yes':'No',safety_defeated_log:closeoutData.safetyDefeatedLog?'Yes':'No',fire_watch_completed:closeoutData.fireWatchCompleted?'Yes':'No',time_permit_closed:closeoutData.timePermitClosed||null,close_out_worker:closeoutData.closeOutWorker,close_out_operator:closeoutData.closeOutOperator,closed_at:new Date().toISOString()});
      if(error)throw error;setCloseoutSuccess(true);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const resetForm=()=>{setFormData({permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),hotWorkType:'',company:'',location:'',phoneRadio:'',toolsEquipment:'',workLocationDetail:'',workDescription:'',zeroEnergyState:'',flowPressureReq:'',flammableAtmosphere:'',fireHalonBypassed:'',remoteShutdowns:'',gasTest1Time:'',gasTest1O2:'',gasTest1LEL:'',gasTest1H2S:'',gasTest1CO:'',gasTest1Other:'',gasTest1Tester:'',gasTest2Time:'',gasTest2O2:'',gasTest2LEL:'',gasTest2H2S:'',gasTest2CO:'',gasTest2Other:'',gasTest2Tester:'',specialPrecautionsNA:'',specialPrecautions:'',safetyReviewedWelding:'',workAreaCompatible:'',correctLineEquipment:'',verifiedZeroEnergy:'',flammablesRemoved:'',remoteAlarmPossible:'',nearHalonWaterMist:'',continualGasMonitoring:'',sumpsDrainsCovered:'',sparkContainment:'',purgeReviewed:'',fireExtinguisher:'',respiratoryProtection:'',fireWatch:'',fireWatchDuration:'',weldingMachineGrounded:'',hosesConnected:'',gasCylindersSecured:'',ventilationAdequate:'',inServiceWeldingPackage:'',utReportVerified:'',hazardCategories:{},hazardTask1:'',hazard1:'',mitigation1:'',hazardTask2:'',hazard2:'',mitigation2:'',hazardTask3:'',hazard3:'',mitigation3:'',workGroupLeader:'',companySafetyRep:'',responsibleSupervisor:'',boardOperator:'',areaOperator:'',startTime:'',expirationTime:'',crewAcknowledgment:''});setSubmitted(false);setSubmittedPermit(null);setPermitCode(null);};
  const resetCloseout=()=>{setCloseoutData({jobCompleted:false,workAreaSecured:false,systemsReturned:false,safetyDefeatedLog:false,fireWatchCompleted:false,timePermitClosed:'',closeOutWorker:'',closeOutOperator:''});setSelectedPermit(null);setCloseoutSuccess(false);setCloseoutCode('');loadOpenPermits();};

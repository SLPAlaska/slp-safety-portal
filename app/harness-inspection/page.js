...
const HARNESS_TYPES = [
  'Full Body Harness', 'Construction Style', 'Vest Style', 'Crossover Style',
  'Positioning Harness', 'Rescue Harness', 'Confined Space Harness', 'Tower Climbing Harness', 'Other'
];

const D_RING_CONFIGS = [
  'Back D-Ring Only', 'Back + Chest D-Rings', 'Back + Side D-Rings',
  'Back + Shoulder D-Rings', 'Full Configuration (All)', 'Other'
];

export default function HarnessInspection() {
  ...
  const checkDuplicateInspection = async (harnessId) => {
    if (!harnessId || duplicateConfirmed) return;

    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    let data = []; try { data = (await fieldData('recent_check', { table: 'harness_inspections', id: harnessId })).rows; } catch (e) { console.error(e); } const error = null;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'harness_id' && value.length >= 3) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await safeSubmit({
        table: 'harness_inspections',
        data: { ...formData },
        photoRef: photoRef,
        formType: 'harness-inspection'
      });
  ...
  const getItemStyle = (value) => {
    if (value === 'Pass' || value === 'Pass - No Fall History') return { borderColor: '#16a34a', background: '#f0fdf4' };
    if (value === 'Fail' || value === 'Fail - Fall Exposure Reported') return { borderColor: '#dc2626', background: '#fef2f2' };
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 50%, #b91c1c 100%)', padding: '20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '50px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '40px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '25px' }}>Harness Inspection recorded successfully.</p>
            <button
              onClick={resetForm}
              style={{
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)', overflow: 'hidden' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', color: 'white', padding: '30px 20px', textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: '28px', marginBottom: '5px', textShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>🦺 Fall Protection Harness Inspection</h1>
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '8px', padding: '10px 15px', marginTop: '15px', fontSize: '13px' }}>
              ⚠️ OSHA 29 CFR 1926.502 & ANSI Z359.1 Compliant | Monthly Inspection Required
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '25px' }}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #1e3a8a' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Harness ID / Serial Number *</label>
                  <input type="text" name="harness_id" value={formData.harness_id} onChange={handleChange} required
                    placeholder="e.g., FPH-001 or S/N 12345"
                    onBlur={() => checkDuplicateInspection(formData.harness_id)}
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                  ...
                  {duplicateWarning && !duplicateConfirmed && (
                    <div style={{ background: 'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)', border: '2px solid #f59e0b', borderRadius: '8px', padding: '15px', marginTop: '10px' }}>
                      ...
                      <p style={{ color: '#92400e', fontSize: '13px', marginBottom: '10px' }}>
                        This harness was inspected <strong>{duplicateWarning.daysAgo} days ago</strong> on {duplicateWarning.date} by {duplicateWarning.inspector}.
                      </p>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <button type="button" onClick={() => { setFormData(prev => ({ ...prev, harness_id: '' })); setDuplicateWarning(null); }}
                ...
                <div>
                  ...
                  <input type="text" name="assigned_user" value={formData.assigned_user} onChange={handleChange}
                    placeholder="Name of person assigned this harness"
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }} />
                </div>
                <div>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>Harness Type *</label>
                  <select name="harness_type" value={formData.harness_type} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontWeight: '600', color: '#374151', marginBottom: '5px', fontSize: '14px' }}>D-Ring Configuration *</label>
                  <select name="d_ring_config" value={formData.d_ring_config} onChange={handleChange} required
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '16px' }}>
                    <option value="">Select Configuration...</option>
                    {D_RING_CONFIGS.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #ea580c' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="Webbing - No Chemical Damage" name="webbing_chemical" />
                <InspectionItem label="Webbing - No UV Degradation" name="webbing_uv" />
                <InspectionItem label="Webbing - No Excessive Wear" name="webbing_wear" />
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #7c3aed' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="Buckle Condition (No Damage)" name="buckle_condition" />
                <InspectionItem label="Keepers/Loops Intact" name="keepers_intact" />
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #0891b2' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem label="Sub-Pelvic Strap" name="sub_pelvic_strap" showNA />
                <InspectionItem label="Padding Condition" name="padding_condition" showNA />
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '20px', marginBottom: '20px', borderLeft: '4px solid #dc2626' }}>
              <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#dc2626', marginBottom: '15px' }}>
                ⚠️ Safety Features & Fall History
              </h3>
              <div style={{ background: 'linear-gradient(135deg, #fee2e2 0%, #fecaca 100%)', border: '2px solid #dc2626', borderRadius: '8px', padding: '12px 15px', marginBottom: '15px' }}>
                <p style={{ fontSize: '13px', color: '#991b1b', margin: 0 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                ...
                <InspectionItem 
                  label="No Previous Fall Exposure" 
                  name="previous_fall" 
                  options={[
                    { value: 'Pass - No Fall History', label: 'Pass - No Fall History' },
                    { value: 'Fail - Fall Exposure Reported', label: 'Fail - Fall Exposure Reported' }
                  ]}
                />
              </div>
            </div>
            ...
            {/* Photo Documentation */}
            <MultiPhotoUpload ref={photoRef} formType="harness-inspection" />

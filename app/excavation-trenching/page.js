...
export default function ExcavationPermit(){
  ...
  const [permitCode,setPermitCode]=useState(null);
  const [closeoutCode,setCloseoutCode]=useState('');
  ...
  const [formData,setFormData]=useState({
    permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),durationOfWork:'',company:'',location:'',phoneRadio:'',excavationLocationDesc:'',purposeOfExcavation:'',
    ...
    workGroupLeader:'',competentPersonSignature:'',areaOperator:'',responsibleSupervisor:'',startDate:new Date().toLocaleDateString('en-CA'),expirationDate:getDefaultExpDate(),crewAcknowledgment:''
  });
  ...
  const loadOpenPermits=async()=>{try{const{rows}=await fieldData('open_permits',{table:'excavation_permits'});setOpenPermits(rows||[]);}catch(e){console.error(e);}};
  ...
  const handleSubmit=async(e)=>{e.preventDefault();setIsSubmitting(true);
    try{
      ...
      const{error}=await safeInsert('excavation_permits', [{
  ...
  const submitCloseout=async()=>{if(!selectedPermit){alert('Select a permit');return;}if(!closeoutCode.trim()){alert('Enter the Close-Out Code from the permit receipt');return;}if(!closeoutData.closeOutWorker||!closeoutData.closeOutSupervisor){alert('Fill required fields');return;}
    setIsSubmitting(true);try{
      const{error}=await safeCloseout('excavation_permits',selectedPermit,closeoutCode,{permit_status:'Closed',job_completed:closeoutData.jobCompleted?'Yes':'No',excavation_backfilled:closeoutData.excavationBackfilled?'Yes':'No',area_restored:closeoutData.areaRestored?'Yes':'No',utilities_verified:closeoutData.utilitiesVerified?'Yes':'No',time_permit_closed:closeoutData.timePermitClosed||null,close_out_worker:closeoutData.closeOutWorker,close_out_supervisor:closeoutData.closeOutSupervisor,closed_at:new Date().toISOString()});
  ...
  if(submitted&&submittedPermit){return(<div style={s.container}><div style={{maxWidth:'600px',margin:'0 auto',paddingTop:'50px'}}><div style={s.successMessage}><div style={{fontSize:'48px',marginBottom:'15px'}}>✅</div><h2 style={{margin:'0 0 10px'}}>Permit Issued Successfully!</h2><div style={{background:'rgba(255,255,255,0.2)',padding:'12px',borderRadius:'8px',fontSize:'20px',fontWeight:'bold',margin:'15px 0'}}>{submittedPermit}</div>{permitCode&&<div style={{background:'white',color:'#b91c1c',padding:'14px',borderRadius:'8px',margin:'0 0 15px'}}><div style={{fontSize:'12px',fontWeight:'700',letterSpacing:'1px'}}>CLOSE-OUT CODE — REQUIRED TO CLOSE THIS PERMIT</div><div style={{fontSize:'30px',fontWeight:'800',letterSpacing:'6px',margin:'6px 0'}}>{permitCode}</div><div style={{fontSize:'11px',color:'#374151'}}>Write it on the printed permit or take a photo of this screen.</div></div>}<p>Daily inspections must be conducted by competent person.</p><div style={{display:'flex',gap:'10px',justifyContent:'center',marginTop:'20px',flexWrap:'wrap'}}><button onClick={resetForm} style={{...s.submitBtn,maxWidth:'200px',background:'white',color:'#059669'}}>Issue Another</button><a href="https://portal.slpalaska.com" style={{...s.submitBtn,maxWidth:'200px',background:'#6b7280',textDecoration:'none',textAlign:'center'}}>Back to Portal</a></div></div></div></div>);}

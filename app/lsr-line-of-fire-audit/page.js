...
export default function LSRLineOfFireAudit() {
  const [formData, setFormData] = useState({
    ...
    // Pinch & Crush Hazards
    pinch_points_idd: '',
    tools_minimize_pinch: '',
    crush_areas_barricaded: '',
    procedures_loto: '',
    // Training & Awareness
    workers_trained: '',
    setup_prevents: '',
  ...
  const uploadPhoto = async () => {
    ...
    const fileName = `lsr-line-of-fire-${Date.now()}.${fileExt}`;
    const filePath = `lsr-line-of-fire-audit/${fileName}`;
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', padding: '20px' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '50px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '40px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.4)' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '25px' }}>LSR-Line of Fire Audit recorded successfully.</p>
            ...
            <div style={{ 
              ...
              <AddToSailLog
                sourceForm="lsr-line-of-fire-audit"
                prefillData={{
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)', color: 'white', padding: '25px', textAlign: 'center' }}>
            ...
            <h1 style={{ margin: '0', fontSize: '24px' }}>LSR-Line of Fire Audit</h1>
            <p style={{ margin: '8px 0 0', opacity: 0.9, fontSize: '13px' }}>Life Saving Rules - Struck-by, Caught-between & Crush Hazard Prevention</p>
          ...
          <form onSubmit={handleSubmit} style={{ padding: '25px' }}>
            ...
            {/* Work Area & SIMOPS */}
            <SectionHeader icon="🎯" color="secondary">Work Area & SIMOPS</SectionHeader>
            ...
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '6px', padding: '12px', marginBottom: '15px', fontSize: '12px', color: '#991b1b' }}>
              <strong>Life Saving Rule:</strong> Line of Fire incidents (struck-by, caught-between, crush) are a leading cause of serious injuries. Identify and control all LOF hazards before starting work.
            </div>
            
            <AuditQuestion label="Work Area Free of Unnecessary Objects Posing Risk?" name="work_area_free" />
            <AuditQuestion label="SIMOPS that could risk Line of Fire Incidents Nearby?" name="simops_risk" />
            <AuditQuestion label="Worksite Clean/Organized to Prevent LOF Risks?" name="worksite_clean" />

            {/* Pinch & Crush Hazards */}
            <SectionHeader icon="✋" color="primary">Pinch & Crush Hazards</SectionHeader>
            
            <AuditQuestion label="Pinch Points ID'd, Marked and/or Labeled?" name="pinch_points_idd" />
            <AuditQuestion label="Tools/Equipment used to minimize pinch/caught hazards?" name="tools_minimize_pinch" />
            <AuditQuestion label="Potential Crush Areas Barricaded?" name="crush_areas_barricaded" />
            <AuditQuestion label="Procedures to LOTO equipment - prevent crush/pinch?" name="procedures_loto" />

            {/* Training & Awareness */}
            <SectionHeader icon="📚" color="secondary">Training & Awareness</SectionHeader>
            
            <AuditQuestion label="All workers trained on crush/pinch ID & Avoidance?" name="workers_trained" />
            <AuditQuestion label="Setup prevents being caught between moving objects?" name="setup_prevents" />

            {/* Exclusion Zones & Equipment */}
            <SectionHeader icon="🚧" color="primary">Exclusion Zones & Equipment</SectionHeader>
            
            <AuditQuestion label="Effective Exclusion Zones in Place?" name="exclusion_zones" />
            <AuditQuestion label="Equipment Operators Avoid Creating Struck-by Hazards?" name="operators_avoid" />

            {/* PPE & Emergency Preparedness */}
            <SectionHeader icon="🦺" color="secondary">PPE & Emergency Preparedness</SectionHeader>
            
            <AuditQuestion label="Workers wearing PPE to protect against LOF risks?" name="workers_ppe" />
            <AuditQuestion label="Emergency Response Procedures Understood by All?" name="emergency_procedures" />
            ...
            >
              {submitting ? 'Submitting...' : 'Submit Line of Fire Audit'}
            </button>
          </form>

...
const handleSubmit=async(e)=>{e.preventDefault();if(!qualityCheckPassed){alert('Run quality check first.');return;}
    setIsSubmitting(true);try{
      ...
      if(stepsData.length>0){const{error:stepsErr}=await supabase.from('tha_task_steps').insert(stepsData);if(stepsErr){console.error('Steps insert error:',stepsErr);}}
      ...
      if(crewData.length>0){const{error:crewErr}=await supabase.from('tha_crew_signoff').insert(crewData);if(crewErr){console.error('Crew insert error:',crewErr);}}

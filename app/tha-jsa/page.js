...
function analyzeQuality(taskSteps) {
  ...
  taskSteps.forEach((step,i)=>{
    ...
    else if(step.task.split(/\s+/).length>=8){stepScore+=5;stepFeedback.push('✓ Good detail');}
    ...
    if(!step.actions?.trim()){stepScore-=8;stepFeedback.push('Add controls');}
      ...
      if(goodCount>=2){stepScore+=10;hasGoodControls=true;results.summary.goodControlsCount++;stepFeedback.push('✅ Good controls!');}
      ...
      if(hasAwareness&&!hasGoodControls){results.summary.awarenessOnlyCount++;stepScore-=5;stepFeedback.push('💡 Add physical controls');}
    }
  ...
  if(results.overallScore>=90){results.feedback.push('✅ EXCELLENT THA!');results.strengths.push('Thorough hazard identification','Strong physical controls');}
  else if(results.overallScore>=80){results.feedback.push('✓ GOOD THA!');results.strengths.push('Good hazard awareness');}
  else if(results.overallScore>=70)results.feedback.push('✓ DECENT THA');
  else results.feedback.push('⚠️ Add more detail');
  if(results.summary.awarenessOnlyCount>0)results.improvements.push('💡 '+results.summary.awarenessOnlyCount+' step(s) need physical controls');
  if(taskSteps.length<3)results.improvements.push('💡 Add more steps (3-5+)');
  return results;
}
...
export default function THAForm(){
  ...
  const [thaCode,setThaCode]=useState(null);
  const [closeoutThaCode,setCloseoutThaCode]=useState('');
...
const handleSubmit=async(e)=>{e.preventDefault();if(!qualityCheckPassed){alert('Run quality check first.');return;}
    setIsSubmitting(true);try{
      ...
      if(stepsData.length>0){const{error:stepsErr}=await safeInsert('tha_task_steps', stepsData, 'tha-jsa');if(stepsErr){console.error('Steps insert error:',stepsErr);}}
      ...
      if(crewData.length>0){const{error:crewErr}=await safeInsert('tha_crew_signoff', crewData, 'tha-jsa');if(crewErr){console.error('Crew insert error:',crewErr);}}
      await registerRecordKey('tha_assessments',thaNumber,code);setThaCode(code);setSubmittedTHA({thaNumber,qualityGrade:q.overallGrade,qualityScore:q.overallScore,feedback:q.feedback.join(' ')});setSubmitted(true);
  ...
  const saveUpdates=async()=>{if(!selectedTHA)return;
    ...
    setIsSubmitting(true);try{
      ...
      if(hasSteps){const stepsData=newTaskSteps.filter(s=>s.task||s.hazards||s.actions).map((s,i)=>({tha_number:selectedTHA,step_number:maxStep+i+1,task_step:s.task,potential_hazards:s.hazards,recommended_actions:s.actions}));if(stepsData.length>0)await safeInsert('tha_task_steps', stepsData, 'tha-jsa');}
      if(hasCrew){const crewData=newCrewMembers.filter(c=>c.name).map(c=>({tha_number:selectedTHA,crew_member_name:c.name,company:c.company,comments:c.comments}));if(crewData.length>0)await safeInsert('tha_crew_signoff', crewData, 'tha-jsa');}
      if(thaCode){await safeCloseout('tha_assessments',selectedTHA,thaCode,{last_modified:new Date().toISOString()});}
      setUpdateSuccess(true);setTimeout(()=>loadTHADetails(selectedTHA),1500);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const submitCloseout=async()=>{if(!closeoutThaNumber){alert('Enter THA number');return;}if(!closeoutThaCode.trim()){alert('Enter the THA Code shown when the THA was created');return;}if(!closeoutData.closed_by){alert('Enter your name');return;}
    setIsSubmitting(true);try{
      const{error:closeErr}=await safeCloseout('tha_assessments',closeoutThaNumber,closeoutThaCode,{status:'Closed',scope_changed:closeoutData.scope_changed,tha_updated_if_changed:closeoutData.tha_updated_if_changed,client_rep_visited:closeoutData.client_rep_visited?'Yes':'No',hse_visited:closeoutData.hse_visited?'Yes':'No',job_stopped:closeoutData.job_stopped?'Yes':'No',housekeeping_complete:closeoutData.housekeeping_complete?'Yes':'No',all_permits_closed:closeoutData.all_permits_closed?'Yes':'No',info_passed_to_oncoming:closeoutData.info_passed_to_oncoming?'Yes':'No',aar_went_well:closeoutData.aar_went_well,aar_didnt_go_as_planned:closeoutData.aar_didnt_go_as_planned,aar_how_improve:closeoutData.aar_how_improve,lessons_learned:closeoutData.lessons_learned,crew_comments:closeoutData.crew_comments,closed_by:closeoutData.closed_by,closed_at:new Date().toISOString()});if(closeErr)throw closeErr;
      setCloseoutSuccess(true);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const resetForm=()=>{setFormData({date:new Date().toLocaleDateString('en-CA'),company:'',location:'',work_area:'',crew_lead:'',phone_radio:'',task_description:'',job_planning_notes:'',procedure_exists:'',procedure_reviewed:'',driving_backing:false,walkaround_360:false,seatbelts_checked:false,spotter_when_backing:false,load_secured:false,general_items:{},ppe_requirements:{},permits_required:{},potential_hazards:{},energy_types:[],control_types:[]});setTaskSteps([{task:'',hazards:'',actions:''},{task:'',hazards:'',actions:''},{task:'',hazards:'',actions:''}]);setCrewMembers([{name:'',company:'',comments:''}]);setSubmitted(false);setSubmittedTHA(null);setQualityCheckPassed(false);setQualityResults(null);setShowQualityPanel(false);setCrewUnlocked(false);};
  const resetCloseout=()=>{setCloseoutThaNumber('');setCloseoutThaCode('');setCloseoutData({scope_changed:'',tha_updated_if_changed:'',client_rep_visited:false,hse_visited:false,job_stopped:false,housekeeping_complete:false,all_permits_closed:false,info_passed_to_oncoming:false,aar_went_well:'',aar_didnt_go_as_planned:'',aar_how_improve:'',lessons_learned:'',crew_comments:'',closed_by:''});setCloseoutSuccess(false);loadOpenTHAs();};
  ...
  if(submitted&&submittedTHA){return(<div style={s.container}><div style={{maxWidth:'600px',margin:'0 auto',paddingTop:'50px'}}><div style={s.successMessage}><h2 style={{margin:'0 0 10px',fontSize:'24px'}}>✓ THA Created!</h2><div style={{background:'rgba(255,255,255,0.2)',padding:'8px 16px',borderRadius:'6px',display:'inline-block',margin:'10px 0',fontSize:'20px',fontWeight:'bold'}}>{submittedTHA.thaNumber}</div><div style={{margin:'15px 0'}}><div style={{fontSize:'48px',fontWeight:'bold',color:gradeColors[submittedTHA.qualityGrade]}}>{submittedTHA.qualityGrade}</div><div style={{fontSize:'18px'}}>Score: {submittedTHA.qualityScore}%</div></div>{thaCode&&<div style={{background:'white',color:'#b91c1c',padding:'14px',borderRadius:'8px',margin:'0 0 15px'}}><div style={{fontSize:'12px',fontWeight:'700',letterSpacing:'1px'}}>THA CODE — REQUIRED TO CLOSE OUT</div><div style={{fontSize:'30px',fontWeight:'800',letterSpacing:'6px',margin:'6px 0'}}>{thaCode}</div><div style={{fontSize:'11px',color:'#374151'}}>Write it on the THA or take a photo of this screen.</div></div>}<p>THA is now OPEN. Close out at end of shift.</p><div style={{display:'flex',gap:'10px',justifyContent:'center',marginTop:'20px',flexWrap:'wrap'}}><button onClick={resetForm} style={{...s.submitBtn,maxWidth:'200px',background:'white',color:'#059669'}}>Create New THA</button><a href="https://portal.slpalaska.com" style={{...s.submitBtn,maxWidth:'200px',background:'#6b7280',textDecoration:'none',display:'inline-block',textAlign:'center'}}>Back to Portal</a></div></div></div></div>);}

  return(<div style={s.container}><div style={s.formContainer}>
    <div style={s.header}><a href="https://portal.slpalaska.com" style={{color:'white',textDecoration:'none',fontSize:'14px'}}>← Back to Portal</a><div style={{background:'rgba(255,255,255,0.95)',borderRadius:'12px',padding:'15px',width:'fit-content',margin:'15px auto',boxShadow:'0 4px 15px rgba(0,0,0,0.2)'}}><img src="/Logo.png" alt="SLP Alaska" style={{maxWidth:'200px',height:'auto'}}/></div><h1 style={{margin:'0 0 8px',fontSize:'24px'}}>Task Hazard Assessment (THA)</h1><p style={{opacity:0.9,fontSize:'13px'}}>Job Safety Analysis with Real-Time Quality Coaching</p><div style={{display:'inline-block',background:'white',color:'#1e3a8a',padding:'4px 12px',borderRadius:'15px',fontSize:'11px',fontWeight:'600',marginTop:'8px'}}>📋 SWISS CHEESE MODEL VALIDATION</div></div>
    <div style={s.content}>
      <div style={s.tabs}><div style={{...s.tab,...(activeTab==='new'?s.tabActive:{})}} onClick={()=>setActiveTab('new')}>📝 New THA</div><div style={{...s.tab,background:'#fef3c7',borderRadius:'6px 6px 0 0',...(activeTab==='open'?{...s.tabActive,background:'#fcd34d'}:{})}} onClick={()=>setActiveTab('open')}>📂 My Open THAs</div><div style={{...s.tab,...(activeTab==='closeout'?s.tabActive:{})}} onClick={()=>setActiveTab('closeout')}>✅ Close Out THA</div></div>

      {activeTab==='new'&&(<form onSubmit={handleSubmit}>
        <div style={{...s.sectionHeader,...s.sectionBlue,marginTop:0}}>📋 General Information</div>
        ...
        <div style={{...s.sectionHeader,...s.sectionYellow,color:'#000'}}>🚗 Driving / Backing</div>
        ...
        <div style={{...s.sectionHeader,...s.sectionRed}}>⚡ Energy Wheel</div>
        <div style={s.energySection}><div style={{color:'#991b1b',fontWeight:'bold',marginBottom:'10px'}}>⚡ Energy Types Present</div><div style={s.energyGrid}>{ENERGY_TYPES.map(e=><div key={e.value} style={{...s.energyItem,...(formData.energy_types.includes(e.value)?s.energyItemSelected:{})}} onClick={()=>toggleEnergyType(e.value)}><span style={{fontSize:'20px'}}>{e.icon}</span><div><div style={{fontWeight:'500',fontSize:'13px'}}>{e.value}</div><div style={{fontSize:'11px',color:'#666'}}>{e.desc}</div></div></div>)}</div><div style={{marginTop:'20px',paddingTop:'15px',borderTop:'1px solid #fecaca'}}><div style={{color:'#065f46',fontWeight:'bold',marginBottom:'10px'}}>🛡️ Control Types</div><div style={s.controlGrid}>{CONTROL_TYPES.map(c=><div key={c.value} style={{...s.controlItem,...(formData.control_types.includes(c.value)?s.controlItemSelected:{})}} onClick={()=>toggleControlType(c.value)}><span>{c.icon}</span><span>{c.value}</span></div>)}</div></div></div>

        <div style={{...s.sectionHeader,...s.sectionBlue}}>☑️ Pre-Task Checklists</div>
      ...
      {activeTab==='closeout'&&<div>{closeoutSuccess?<div style={s.successMessage}><h2 style={{margin:'0 0 10px'}}>✓ THA Closed!</h2><button type="button" onClick={resetCloseout} style={{...s.submitBtn,maxWidth:'250px',background:'white',color:'#059669',marginTop:'15px'}}>Close Another</button></div>:<><div style={{...s.sectionHeader,...s.sectionGreen,marginTop:0}}>✅ Close Out THA</div><div style={s.formGroup}><label style={s.label}>THA Number *</label><input type="text" value={closeoutThaNumber} onChange={e=>setCloseoutThaNumber(e.target.value)} placeholder="THA-XXXXXXXX-XXXX" style={s.input}/></div><div style={s.formGroup}><label style={s.label}>THA Code *</label><input type="text" value={closeoutThaCode} onChange={e=>setCloseoutThaCode(e.target.value.toUpperCase())} placeholder="From THA creation screen" maxLength={8} style={{...s.input,letterSpacing:'3px',fontWeight:'700'}}/></div>{openTHAs.length>0&&<div style={{marginBottom:'20px'}}><p style={{fontSize:'13px',marginBottom:'10px'}}><strong>Or select:</strong></p><div style={{maxHeight:'150px',overflowY:'auto',border:'1px solid #e5e7eb',borderRadius:'6px'}}>{openTHAs.map(t=><div key={t.tha_number} style={{padding:'8px',borderBottom:'1px solid #e5e7eb',cursor:'pointer',fontSize:'12px'}} onClick={()=>setCloseoutThaNumber(t.tha_number)}><strong>{t.tha_number}</strong> - {t.company}</div>)}</div></div>}<div style={s.closeoutSection}><h4 style={{margin:'0 0 15px',color:'#065f46'}}>📋 Post-Task Review</h4><div style={s.row}><div style={s.formGroup}><label style={s.label}>Scope changed?</label><div style={s.radioGroup}>{['Yes','No'].map(o=><div key={o} style={{...s.radioOption,...(closeoutData.scope_changed===o?s.radioOptionSelected:{})}} onClick={()=>setCloseoutData(p=>({...p,scope_changed:o}))}><input type="radio" checked={closeoutData.scope_changed===o} readOnly/><span>{o}</span></div>)}</div></div><div style={s.formGroup}><label style={s.label}>THA updated?</label><div style={s.radioGroup}>{['Yes','No','N/A'].map(o=><div key={o} style={{...s.radioOption,...(closeoutData.tha_updated_if_changed===o?s.radioOptionSelected:{})}} onClick={()=>setCloseoutData(p=>({...p,tha_updated_if_changed:o}))}><input type="radio" checked={closeoutData.tha_updated_if_changed===o} readOnly/><span>{o}</span></div>)}</div></div></div></div><div style={{...s.sectionHeader,...s.sectionCyan}}>☑️ End of Shift</div><div style={s.closeoutChecklist}>{[{key:'client_rep_visited',label:'👔 Client Rep visited?'},{key:'hse_visited',label:'🦺 HSE visited?'},{key:'job_stopped',label:'🛑 Job stopped?'},{key:'housekeeping_complete',label:'🧹 Housekeeping complete?'},{key:'all_permits_closed',label:'📝 Permits closed?'},{key:'info_passed_to_oncoming',label:'🔄 Info passed?'}].map(item=><label key={item.key} style={{...s.closeoutCheckItem,...(closeoutData[item.key]?s.closeoutCheckItemChecked:{})}} onClick={()=>setCloseoutData(p=>({...p,[item.key]:!p[item.key]}))}><input type="checkbox" checked={closeoutData[item.key]} readOnly style={{width:'20px',height:'20px'}}/><span style={{fontSize:'13px'}}>{item.label}</span></label>)}</div><div style={{...s.sectionHeader,background:'linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)'}}>🎯 After Action Review</div><div style={s.aarSection}><div style={{marginBottom:'15px'}}><label style={{...s.label,fontWeight:'600',color:'#1e40af'}}>1. What went well? ✅</label><textarea value={closeoutData.aar_went_well} onChange={e=>setCloseoutData(p=>({...p,aar_went_well:e.target.value}))} placeholder="What worked?" style={{...s.textarea,minHeight:'80px'}}/></div><div style={{marginBottom:'15px'}}><label style={{...s.label,fontWeight:'600',color:'#1e40af'}}>2. What didn't go as planned? ⚠️</label><textarea value={closeoutData.aar_didnt_go_as_planned} onChange={e=>setCloseoutData(p=>({...p,aar_didnt_go_as_planned:e.target.value}))} placeholder="Challenges?" style={{...s.textarea,minHeight:'80px'}}/></div><div><label style={{...s.label,fontWeight:'600',color:'#1e40af'}}>3. How will we improve? 📈</label><textarea value={closeoutData.aar_how_improve} onChange={e=>setCloseoutData(p=>({...p,aar_how_improve:e.target.value}))} placeholder="Improvements?" style={{...s.textarea,minHeight:'80px'}}/></div></div><div style={{...s.sectionHeader,...s.sectionPurple}}>📝 Additional Info</div><div style={s.formGroup}><label style={s.label}>Lessons Learned</label><textarea value={closeoutData.lessons_learned} onChange={e=>setCloseoutData(p=>({...p,lessons_learned:e.target.value}))} style={s.textarea}/></div><div style={s.formGroup}><label style={s.label}>Crew Comments</label><textarea value={closeoutData.crew_comments} onChange={e=>setCloseoutData(p=>({...p,crew_comments:e.target.value}))} style={s.textarea}/></div><div style={s.formGroup}><label style={s.label}>Closed By *</label><input type="text" value={closeoutData.closed_by} onChange={e=>setCloseoutData(p=>({...p,closed_by:e.target.value}))} placeholder="Your name" style={s.input}/></div><button type="button" onClick={submitCloseout} disabled={isSubmitting} style={{...s.submitBtn,background:'linear-gradient(135deg, #059669 0%, #047857 100%)',opacity:isSubmitting?0.5:1}}>{isSubmitting?'Closing...':'✅ Close Out THA'}</button></>}</div>}
    </div>

...
export default function EnergyIsolation(){
  ...
  const [formData,setFormData]=useState({
    permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),lockoutType:'',company:'',location:'',phoneRadio:'',workArea:'',equipmentId:'',workDescription:'',authorizedPersonName:'',authorizedPersonLock:'',
  ...
  const handleSubmit=async(e)=>{e.preventDefault();setIsSubmitting(true);
    try{
      ...
      const{error}=await safeInsert('ei_permits', [{
  ...
  const signOutWorker=async(workerId,workerName)=>{setIsSubmitting(true);try{
      const codeEntry=closeoutCode.trim()||window.prompt('Enter the Close-Out Code from the permit receipt to sign out '+workerName)||'';if(!codeEntry.trim()){setIsSubmitting(false);return;}
      const{error}=await safeCloseout('ei_worker_log',workerId,codeEntry,{sign_out_time:new Date().toISOString()},workerSelectedPermit);
      if(error)throw error;
      loadActiveWorkers(workerSelectedPermit);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const submitCloseout=async()=>{if(!selectedPermit){alert('Select a permit');return;}if(!closeoutCode.trim()){alert('Enter the Close-Out Code from the permit receipt');return;}if(!closeoutData.closeOutBy){alert('Close out by required');return;}
    if(activeWorkers.length>0){alert('Workers still signed in!');return;}
    setIsSubmitting(true);try{
      const{error}=await safeCloseout('ei_permits',selectedPermit,closeoutCode,{permit_status:'Closed',all_workers_signed_out:closeoutData.allWorkersSignedOut?'Yes':'No',personal_locks_removed:closeoutData.personalLocksRemoved?'Yes':'No',device_locks_removed:closeoutData.deviceLocksRemoved?'Yes':'No',tags_removed_matched:closeoutData.tagsRemovedMatched?'Yes':'No',equipment_clear:closeoutData.equipmentClear?'Yes':'No',safe_to_reenergize:closeoutData.safeToReenergize?'Yes':'No',time_permit_closed:closeoutData.timePermitClosed||null,close_out_by:closeoutData.closeOutBy,closed_at:new Date().toISOString()});
      if(error)throw error;setCloseoutSuccess(true);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const resetForm=()=>{setFormData({permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),lockoutType:'',company:'',location:'',phoneRadio:'',workArea:'',equipmentId:'',workDescription:'',authorizedPersonName:'',authorizedPersonLock:'',energySources:{},energyDetails:{},totalIsolationPoints:0,electricalIsolations:0,valveIsolations:0,blindIsolations:0,otherIsolations:0,lockboxUsed:'',lockboxLocation:'',controlLockColor:'',controlLockNumber:'',deviceLockSet1:'',deviceLockSet2:'',deviceLockSet3:'',zeroEnergyVerified:'',verificationMethod:'',tryTestPerformed:'',storedEnergyReleased:'',allDevicesTagged:'',tagStubsInLockbox:'',preJobBriefing:'',hazardsCommunicated:'',emergencyProcedures:'',affectedPersonnelNotified:'',authorizedPersonSignature:'',supervisorApproval:'',startTime:'',expirationTime:''});setSubmitted(false);setSubmittedPermit(null);setPermitCode(null);};
  const resetCloseout=()=>{setCloseoutData({allWorkersSignedOut:false,personalLocksRemoved:false,deviceLocksRemoved:false,tagsRemovedMatched:false,equipmentClear:false,safeToReenergize:false,timePermitClosed:'',closeOutBy:''});setSelectedPermit(null);setCloseoutSuccess(false);setCloseoutCode('');setActiveWorkers([]);loadOpenPermits();};

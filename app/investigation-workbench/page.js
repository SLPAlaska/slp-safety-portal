...
const SLP_TEAM = [
  { name: 'Brian', email: 'brian@slpalaska.com' },
  { name: 'Daniel', email: 'daniel@slpalaska.com' },
  { name: 'Mick', email: 'mick@slpalaska.com' },
  { name: 'Lonnie', email: 'lonnie@slpalaska.com' },
  { name: 'Todd', email: 'todd@slpalaska.com' },
  { name: 'Krystal', email: 'krystal@slpalaska.com' },
  { name: 'Britney', email: 'britney@slpalaska.com' }
];

const ROOT_CAUSE_CATEGORIES = [
  { id: 'human', name: 'Human Factors', causes: ['Inattention/Distraction', 'Fatigue', 'Lack of Knowledge', 'Failure to Follow Procedure', 'Complacency', 'Risk Tolerance'] },
  { id: 'equipment', name: 'Equipment/Machinery', causes: ['Mechanical Failure', 'Wear and Tear', 'Improper Maintenance', 'Equipment Not Fit for Purpose'] },
  { id: 'environmental', name: 'Environmental', causes: ['Weather Conditions', 'Poor Lighting', 'Noise', 'Temperature Extremes', 'Confined Space'] },
  { id: 'organizational', name: 'Organizational', causes: ['Inadequate Resources', 'Production Pressure', 'Inadequate Safety Culture', 'Poor Risk Assessment'] },
  { id: 'procedural', name: 'Procedural', causes: ['Missing Procedure', 'Unclear Procedure', 'Outdated Procedure', 'Inadequate Hazard Assessment', 'Procedure Not Followed'] },
  { id: 'design', name: 'Design/Engineering', causes: ['Design Flaw', 'Inadequate Engineering Controls', 'Ergonomic Issues', 'Inadequate Guarding'] },
  { id: 'communication', name: 'Communication', causes: ['Communication Breakdown', 'Incomplete Handoff', 'Language Barrier', 'Information Not Shared'] },
  { id: 'training', name: 'Training/Competency', causes: ['Inadequate Training', 'Skill/Knowledge Gap', 'No Competency Verification', 'Inadequate Orientation'] },
  { id: 'supervision', name: 'Supervision', causes: ['Inadequate Supervision', 'Supervisor Not Present', 'Failed to Correct Behavior'] },
  { id: 'external', name: 'External Factors', causes: ['Third Party Actions', 'Regulatory Requirements'] }
];
...
const styles = {
  ...
  warningBox: { background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: '8px', padding: '15px', marginBottom: '20px', color: '#92400e' },
  ...
  async function fetchIncidentData() {
    ...
    try {
      ...
      const [{ data: lr }, { data: fw }, { data: rca }, { data: tl }, { data: ev }, { data: wi }, { data: ca }, { data: ll }] = await Promise.all([
        supabase.from('local_reviews').select('*').eq('incident_id', incidentId).single(),
        supabase.from('five_why_analyses').select('*').eq('incident_id', incidentId).single(),
        supabase.from('rca_analyses').select('*').eq('incident_id', incidentId).single(),
        ...
        <div style={styles.header}>
          <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              {incident.safety_severity && <span style={{ ...styles.badge, background: SEVERITY_COLORS[incident.safety_severity], marginRight: '10px' }}>Severity {incident.safety_severity}</span>}

...
export default function InvestigationWorkbench() {
  ...
  const [newCA, setNewCA] = useState({ action_description: '', hierarchy_control: 'Elimination', action_owner_name: '', target_date: '', action_status: 'Open' });
              ...
              <div key={ca.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:'8px',padding:'15px'}}>
                {editingCA===ca.id ? <div style={{display:'grid',gap:'10px'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'10px'}}>
                    <select value={ca.hierarchy_control} onChange={e=>setCorrectiveActions(correctiveActions.map(x=>x.id===ca.id?{...x,hierarchy_control:e.target.value}:x))} style={st.input}><option value="Elimination">Elimination</option><option value="Substitution">Substitution</option><option value="Engineering">Engineering</option><option value="Administrative">Administrative</option><option value="PPE">PPE</option></select>

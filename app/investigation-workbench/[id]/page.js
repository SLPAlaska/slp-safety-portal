...
export default function InvestigationWorkbench() {
  ...
  const [newCA, setNewCA] = useState({ action_description: '', control_hierarchy: 'Elimination', assigned_to: '', due_date: '', action_status: 'Open' });
  ...
  async function addTimelineEvent() { if (!newTimelineEvent.date || !newTimelineEvent.description) { alert('Fill date+description'); return; } setSaving(true); try { const n=timelineEvents.length+1; const { data, error } = await supabase.from('timeline_events').insert({ incident_id:id, sequence_number:n, date:newTimelineEvent.date, time:newTimelineEvent.time||null, description:newTimelineEvent.description, critical:newTimelineEvent.critical, created_by_email:userEmail }).select().single(); if(error)throw error; setTimelineEvents([...timelineEvents,data]); setNewTimelineEvent({date:'',time:'',description:'',critical:false}); await supabase.from('incidents').update({timeline_event_count:n,timeline_developed:true}).eq('id',id); } catch(e){alert(e.message);} finally{setSaving(false);} }
  async function saveTimelineEdit(item) { setSaving(true); try { const{error}=await supabase.from('timeline_events').update({date:item.date,time:item.time,description:item.description,critical:item.critical}).eq('id',item.id); if(error)throw error; setTimelineEvents(timelineEvents.map(e=>e.id===item.id?{...e,...item}:e)); setEditingTimeline(null); } catch(e){alert(e.message);} finally{setSaving(false);} }
  ...
  async function addCA() { if(!newCA.action_description||!newCA.assigned_to){alert('Fill action+owner');return;} setSaving(true); try { const n=correctiveActions.length+1; const{data,error}=await supabase.from('investigation_corrective_actions').insert({incident_id:id,action_number:n,action_description:newCA.action_description,control_hierarchy:newCA.control_hierarchy,assigned_to:newCA.assigned_to,due_date:newCA.due_date||null,action_status:newCA.action_status,created_by_email:userEmail}).select().single(); if(error)throw error; setCorrectiveActions([...correctiveActions,data]); setNewCA({action_description:'',control_hierarchy:'Elimination',assigned_to:'',due_date:'',action_status:'Open'}); await supabase.from('incidents').update({corrective_actions_count:n}).eq('id',id); } catch(e){alert(e.message);} finally{setSaving(false);} }
  async function saveCAEdit(item) { setSaving(true); try { const{error}=await supabase.from('investigation_corrective_actions').update({action_description:item.action_description,control_hierarchy:item.control_hierarchy,assigned_to:item.assigned_to,due_date:item.due_date||null,action_status:item.action_status}).eq('id',item.id); if(error)throw error; setCorrectiveActions(correctiveActions.map(c=>c.id===item.id?{...c,...item}:c)); setEditingCA(null); } catch(e){alert(e.message);} finally{setSaving(false);} }
...
${sorted.length?`<div class="section"><div class="section-title">Timeline (${sorted.length})</div><div class="section-body"><table><thead><tr><th>Date</th><th>Time</th><th>Description</th><th>Critical</th></tr></thead><tbody>${sorted.map(e=>`<tr class="${e.critical?'critical-row':''}"><td>${e.date||''}</td><td>${e.time||'-'}</td><td>${e.description||''}</td><td>${e.critical?'Yes':'No'}</td></tr>`).join('')}</tbody></table></div></div>`:''}
...
${correctiveActions.length?`<div class="section"><div class="section-title">Corrective Actions (${correctiveActions.length})</div><div class="section-body"><table><thead><tr><th>#</th><th>Action</th><th>Control</th><th>Owner</th><th>Due</th><th>Status</th></tr></thead><tbody>${correctiveActions.map((c,i)=>{const h=c.control_hierarchy||'N/A';const hc=h==='Elimination'?'#166534':h==='Substitution'?'#15803d':h==='Engineering'?'#ca8a04':h==='Administrative'?'#ea580c':'#dc2626';return`<tr><td>${i+1}</td><td>${c.action_description||''}</td><td><span class="hierarchy-badge" style="background:${hc}">${h}</span></td><td>${c.assigned_to||'-'}</td><td>${c.due_date||'-'}</td><td>${c.action_status||'Open'}</td></tr>`;}).join('')}</tbody></table></div></div>`:''}
        ...
        {activeTab==='Timeline' && <div>
          ...
          <div style={{background:'#f8fafc',padding:'20px',borderRadius:'12px',marginBottom:'20px'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 2fr auto',gap:'10px',marginBottom:'15px'}}>
              <input type="date" value={newTimelineEvent.date} onChange={e=>setNewTimelineEvent({...newTimelineEvent,date:e.target.value})} style={st.input} />
              <input type="time" value={newTimelineEvent.time} onChange={e=>setNewTimelineEvent({...newTimelineEvent,time:e.target.value})} style={st.input} />
              <input placeholder="Event description" value={newTimelineEvent.description} onChange={e=>setNewTimelineEvent({...newTimelineEvent,description:e.target.value})} style={st.input} />
          ...
          <div style={{display:'grid',gap:'15px'}}>
            {[...timelineEvents].sort((a,b)=>new Date(a.date+' '+(a.time||'00:00'))-new Date(b.date+' '+(b.time||'00:00'))).map(ev=>(
              <div key={ev.id} style={{background:ev.critical?'#fef3c7':'white',border:ev.critical?'2px solid #f59e0b':'1px solid #e2e8f0',borderRadius:'8px',padding:'15px'}}>
                {editingTimeline===ev.id ? <div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 2fr',gap:'10px',marginBottom:'10px'}}>
                    <input type="date" value={ev.date} onChange={e=>setTimelineEvents(timelineEvents.map(x=>x.id===ev.id?{...x,date:e.target.value}:x))} style={st.input} />
                    <input type="time" value={ev.time||''} onChange={e=>setTimelineEvents(timelineEvents.map(x=>x.id===ev.id?{...x,time:e.target.value}:x))} style={st.input} />
                    <input value={ev.description} onChange={e=>setTimelineEvents(timelineEvents.map(x=>x.id===ev.id?{...x,description:e.target.value}:x))} style={st.input} />
                  </div>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:'10px'}}>
                    <strong>{ev.date} {ev.time&&`at ${ev.time}`}</strong>
                    <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
        ...
        {activeTab==='Corrective Actions' && <div>
          ...
          <div style={{background:'#f8fafc',padding:'20px',borderRadius:'12px',marginBottom:'20px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'10px',marginBottom:'15px'}}>
              <select value={newCA.control_hierarchy} onChange={e=>setNewCA({...newCA,control_hierarchy:e.target.value})} style={st.input}><option value="Elimination">Elimination</option><option value="Substitution">Substitution</option><option value="Engineering">Engineering</option><option value="Administrative">Administrative</option><option value="PPE">PPE</option></select>
              <input placeholder="Owner *" value={newCA.assigned_to} onChange={e=>setNewCA({...newCA,assigned_to:e.target.value})} style={st.input} />
              <input type="date" value={newCA.due_date} onChange={e=>setNewCA({...newCA,due_date:e.target.value})} style={st.input} />
              <select value={newCA.action_status} onChange={e=>setNewCA({...newCA,action_status:e.target.value})} style={st.input}><option value="Open">Open</option><option value="In Progress">In Progress</option><option value="Completed">Completed</option></select>
            </div>
              ...
              <div key={ca.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:'8px',padding:'15px'}}>
                {editingCA===ca.id ? <div style={{display:'grid',gap:'10px'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'10px'}}>
                    <select value={ca.control_hierarchy} onChange={e=>setCorrectiveActions(correctiveActions.map(x=>x.id===ca.id?{...x,control_hierarchy:e.target.value}:x))} style={st.input}><option value="Elimination">Elimination</option><option value="Substitution">Substitution</option><option value="Engineering">Engineering</option><option value="Administrative">Administrative</option><option value="PPE">PPE</option></select>
                    <input value={ca.assigned_to||''} onChange={e=>setCorrectiveActions(correctiveActions.map(x=>x.id===ca.id?{...x,assigned_to:e.target.value}:x))} style={st.input} />
                    <input type="date" value={ca.due_date||''} onChange={e=>setCorrectiveActions(correctiveActions.map(x=>x.id===ca.id?{...x,due_date:e.target.value}:x))} style={st.input} />
                    <select value={ca.action_status} onChange={e=>setCorrectiveActions(correctiveActions.map(x=>x.id===ca.id?{...x,action_status:e.target.value}:x))} style={st.input}><option value="Open">Open</option><option value="In Progress">In Progress</option><option value="Completed">Completed</option></select>
                  </div>
                  <SaveCancelBtns onSave={()=>saveCAEdit(ca)} onCancel={()=>{setEditingCA(null);loadAll();}} />
                </div> : <div>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:'10px'}}>
                    <div><span style={{background:'#dbeafe',color:'#1e40af',padding:'4px 12px',borderRadius:'6px',fontSize:'12px',marginRight:'8px'}}>{ca.control_hierarchy}</span><span style={{background:ca.action_status==='Completed'?'#dcfce7':ca.action_status==='In Progress'?'#fef3c7':'#fee2e2',color:ca.action_status==='Completed'?'#15803d':ca.action_status==='In Progress'?'#d97706':'#dc2626',padding:'4px 12px',borderRadius:'6px',fontSize:'12px'}}>{ca.action_status}</span></div>
                    <div style={{display:'flex',gap:'8px',alignItems:'center'}}><span style={{color:'#64748b',fontSize:'13px'}}>Due: {ca.due_date||'—'}</span><EditDeleteBtns onEdit={()=>setEditingCA(ca.id)} onDelete={()=>deleteCA(ca)} /></div>
                  </div>
                  <p style={{margin:'10px 0',color:'#334155'}}>{ca.action_description}</p>
                  <div style={{fontSize:'13px',color:'#64748b'}}>Owner: {ca.assigned_to}</div>
                </div>}
              </div>
            ))}
          </div>

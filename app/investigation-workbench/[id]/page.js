...
${evidence.length?`<div class="section"><div class="section-title">Evidence (${evidence.length})</div><div class="section-body"><table><thead><tr><th>Type</th><th>Description</th><th>Source</th></tr></thead><tbody>${evidence.map(e=>`<tr><td>${e.evidence_type||''}</td><td>${e.event_description||''}</td><td>${e.source||'initial_report'}</td></tr>`).join('')}</tbody></table></div></div>`:''}
        ...
        {activeTab==='Evidence' && <div>
          ...
          {showEvidenceUpload && <div style={{background:'#f8fafc',padding:'20px',borderRadius:'12px',marginBottom:'20px'}}>
            ...
            <div style={{display:'grid',gap:'15px'}}>
              ...
              <div><label style={{display:'block',marginBottom:'5px',fontWeight:'500'}}>Description</label><textarea value={newEvidence.event_description} onChange={e=>setNewEvidence({...newEvidence,description:e.target.value})} placeholder="Describe..." style={{...st.input,minHeight:'80px',resize:'vertical'}} /></div>

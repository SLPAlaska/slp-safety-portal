...
export default function UnitWorkPermit(){
  ...
  const [formData,setFormData]=useState({
    permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),company:'',location:'',phoneRadio:'',toolsEquipment:'',workDescription:'',workGroupLeader:'',
  ...
  const loadOpenPermits=async()=>{try{const{rows}=await fieldData('open_permits',{table:'unit_work_permits'});setOpenPermits(rows||[]);}catch(e){console.error(e);}};
  ...
  const handleSubmit=async(e)=>{e.preventDefault();setIsSubmitting(true);
    try{
      ...
      const{error}=await safeInsert('unit_work_permits', [{
  ...
  const submitCloseout=async()=>{if(!selectedPermit){alert('Select a permit');return;}if(!closeoutCode.trim()){alert('Enter the Close-Out Code from the permit receipt');return;}if(!closeoutData.closeOutBy||!closeoutData.areaOperatorCloseOut){alert('Fill required fields');return;}
    setIsSubmitting(true);try{
      const{error}=await safeCloseout('unit_work_permits',selectedPermit,closeoutCode,{permit_status:'Closed',job_completed:closeoutData.jobCompleted?'Yes':'',work_area_secured:closeoutData.workAreaSecured?'Yes':'',bypassed_systems_restored:closeoutData.bypassedSystemsRestored?'Yes':'',dsd_log_updated:closeoutData.dsdLogUpdated?'Yes':'',time_permit_closed:closeoutData.timePermitClosed||null,close_out_by:closeoutData.closeOutBy,area_operator_close_out:closeoutData.areaOperatorCloseOut,closed_at:new Date().toISOString()});
      if(error)throw error;setCloseoutSuccess(true);
    }catch(e){console.error(e);alert('Error: '+e.message);}finally{setIsSubmitting(false);}
  };

  const resetForm=()=>{setFormData({permitInitiator:'',date:new Date().toLocaleDateString('en-CA'),company:'',location:'',phoneRadio:'',toolsEquipment:'',workDescription:'',workGroupLeader:'',respiratoryRequired:'',respiratoryType:'',hotTapping:'',hydroTesting:'',fireHalonBypassed:'',remoteShutdownsDisabled:'',atmoTestingRequired:'',atmoTests:[{time:'',oxygen:'',lel:'',h2s:'',co:'',other:'',tester:''},{time:'',oxygen:'',lel:'',h2s:'',co:'',other:'',tester:''}],hazardCategories:{},taskHazardMitigations:[{task:'',hazard:'',mitigation:''},{task:'',hazard:'',mitigation:''},{task:'',hazard:'',mitigation:''}],specialPrecautions:'',generalVerification:{},hotTapVerification:{},xrayVerification:{},crewMembers:[{name:'',company:''}],hotWorkPermit:'',confinedSpacePermit:'',energyIsolationPermit:'',processOpeningPermit:'',workGroupLeaderSign:'',supervisorApproval:'',boardOperator:'',areaOperator:'',startTime:'',expirationTime:''});setSubmitted(false);setSubmittedPermit(null);setPermitCode(null);};
  const resetCloseout=()=>{setCloseoutData({jobCompleted:false,workAreaSecured:false,bypassedSystemsRestored:false,dsdLogUpdated:false,timePermitClosed:'',closeOutBy:'',areaOperatorCloseOut:''});setSelectedPermit(null);setCloseoutSuccess(false);setCloseoutCode('');loadOpenPermits();};

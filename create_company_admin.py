...
files['app/lms/company-admin/page.js'] = r"""'use client'
...
function EmployeesTab({ token, companyId }) {
  ...
  const load = useCallback(async () => {
    const res = await fetch('/api/lms/company-admin/users', { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  return (
    <div>
      ...
      <input style={{ ...S.input, maxWidth: '320px', marginBottom: '16px' }} placeholder="Search employees..." value={search} onChange={e => setSearch(e.target.value)} />
      <table style={S.table}>
        ...
        <tbody>
          {filtered.map(u => (
            <tr key={u.id} style={S.tr}>
              ...
              <td style={S.td}><span style={!u.active ? S.badgeGray : u.must_change_pw ? S.badgeOrange : S.badgeGreen}>{!u.active ? 'Inactive' : u.must_change_pw ? 'Pending Login' : 'Active'}</span></td>
              <td style={S.td}>
                <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                  <button style={S.btnSmall} onClick={() => handleResetPassword(u)} disabled={resetting === u.id}>{resetting === u.id ? '...' : 'Reset PW'}</button>
                  {u.active ? <button style={S.btnSmallRed} onClick={() => handleDeactivate(u)}>Deactivate</button> : <button style={S.btnSmall} onClick={() => handleReactivate(u)}>Reactivate</button>}
                </div>
              </td>
            </tr>
          ))}
      ...
      {showModal && (
        <Modal title="Add Employee" onClose={() => { setShowModal(false); setError('') }}>
          ...
          <button style={S.btnPrimary} onClick={handleCreate} disabled={saving || !form.full_name || !form.email || !form.username || !form.password}>{saving ? 'Creating...' : 'Create Account'}</button>
        </Modal>
      )}
    </div>
  )
...
function AssignCoursesTab({ token, companyId }) {
  ...
  async function handleAssign() {
    ...
    setShowModal(false); setForm({ user_id: '', course_ids: [], due_date: '' }); load()
  }
  ...
  function toggleCourse(courseId) {
    setForm(f => {
      const ids = f.course_ids.includes(courseId) ? f.course_ids.filter(id => id !== courseId) : [...f.course_ids, courseId]
      return { ...f, course_ids: ids }
    })
  }

  const alreadyAssigned = new Set(assignments.filter(a => a.user_id === form.user_id).map(a => a.course_id))
  const companyAssignments = assignments.filter(a => users.some(u => u.id === a.user_id))
  ...
  return (
    <div>
      ...
      <div style={S.infoBox}>Individual assignments are in addition to company-wide required courses.</div>
      <br />
      <table style={S.table}>
      ...
      {showModal && (
        <Modal title="Assign Courses" onClose={() => { setShowModal(false); setForm({ user_id: '', course_ids: [], due_date: '' }); setError('') }}>
            ...
            <Field label="Courses * (select one or more)">
              <div style={{ border: '1px solid #ddd', borderRadius: '8px', maxHeight: '260px', overflowY: 'auto' }}>
                {courses.map(c => {
                  ...
                  return (
                    <div key={c.id} onClick={() => !assigned && toggleCourse(c.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', borderBottom: '1px solid #f0f0f0', cursor: assigned ? 'default' : 'pointer', background: selected ? '#e3f2fd' : assigned ? '#f9f9f9' : '#fff', opacity: assigned ? 0.5 : 1 }}>
                      <div style={{ width: '16px', height: '16px', borderRadius: '4px', border: selected ? 'none' : '1.5px solid #ccc', background: selected ? '#1565c0' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {selected && <span style={{ color: '#fff', fontSize: '11px', fontWeight: '700' }}>v</span>}
                      </div>
          ...
          {error && <div style={S.error}>{error}</div>}
          <button style={S.btnPrimary} onClick={handleAssign} disabled={saving || !form.user_id || form.course_ids.length === 0}>{saving ? 'Assigning...' : `Assign ${form.course_ids.length || ''} Course${form.course_ids.length !== 1 ? 's' : ''}`}</button>
        </Modal>
      )}
    </div>
  )
...
function TrainingMatrixTab({ token }) {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch('/api/lms/company-admin/dashboard', { headers: { 'Authorization': `Bearer ${token}` } })
      const data = await res.json()
      setMatrix(data)
      setLoading(false)
    }
  ...
  return (
    <div>
      <div style={S.tabHeader}><h2 style={S.tabTitle}>Training Matrix</h2></div>
        ...
        <table style={{ ...S.table, fontSize: '12px', minWidth: '600px' }}>
          <thead>
            <tr>
              ...
              {courses.map(c => <th key={c.id} style={{ ...S.th, maxWidth: '100px', whiteSpace: 'normal', lineHeight: '1.3' }}>{c.title}{c.is_required ? ' *' : ''}</th>)}
            </tr>
          </thead>
          <tbody>
            {employees.map(emp => (
...
function ReportsTab({ token }) {
  ...
  return (
    <div>
      <div style={S.tabHeader}><h2 style={S.tabTitle}>Reports</h2></div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '480px' }}>
        <div style={{ border: '1px solid #e5e5e5', borderRadius: '10px', padding: '20px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#666', marginBottom: '16px' }}>Full compliance report for all employees and their training status.</div>
          <button style={S.btnPrimary} onClick={downloadMatrix} disabled={downloading}>{downloading ? 'Generating PDF...' : 'Download PDF Report'}</button>
        </div>
      </div>
    </div>
  )
...
export default function CompanyAdminDashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      setToken(tok)
      const res = await fetch('/api/lms/company-admin/me', { headers: { 'Authorization': `Bearer ${tok}` } })
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#f0f2f5' }}><div style={{ color: '#999' }}>Loading...</div></div>
    ...
    <div style={S.page}>
      <div style={S.pageHeader}>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <a href="/lms/dashboard" style={S.backLink}>Back to Dashboard</a>
      ...
      <div style={S.tabBar}>
        {TABS.map(tab => <button key={tab} style={{ ...S.tabBtn, ...(activeTab === tab ? S.tabBtnActive : {}) }} onClick={() => setActiveTab(tab)}>{tab}</button>)}
      </div>
...
files['app/api/lms/company-admin/me/route.js'] = """import { createClient } from '@supabase/supabase-js'
...
files['app/api/lms/company-admin/users/route.js'] = """import { createClient } from '@supabase/supabase-js'
import { NextResponse } from 'next/server'
...
files['app/api/lms/company-admin/reset-password/route.js'] = """import { createClient } from '@supabase/supabase-js'
import { NextResponse } from 'next/server'
